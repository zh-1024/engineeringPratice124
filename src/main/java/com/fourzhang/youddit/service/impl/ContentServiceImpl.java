...
public class ContentServiceImpl implements ContentService {
    ...
    public Result getLabels() {
        List<LabelResponse> ret = new ArrayList<>();
        List<Label> searched = labelMapper.selectList(new LambdaQueryWrapper<>());

        for (Label l : searched) {
            LabelResponse response = new LabelResponse(l.getLabelName(), l.getLabelId());
            ret.add(response);
        }
